...
async function getCartItems(): Promise<CartItem[]> {
  ...
  const cart = await prisma.cart.findFirst({
    ...
    include: {
      items: {
        include: {
          app: { include: { salesPlans: true, paymentMethods: true } },
        },
      },
    },
  });
  ...
  const mapped: CartItem[] = cart.items
    .map((item): CartItem | null => {
      ...
      const supportsWcoin = item.app.paymentMethods.some(
        (pm) => pm.method === "W",
      );
      const supportsCard = item.app.paymentMethods.some(
        (pm) => pm.method === "C",
      );
