...
function normalizeRequestedPath(rawPath: string | null): string {
  const base = rawPath && rawPath.trim() ? rawPath : "index.html";
  const noLeadingSlash = base.replace(/^\/+/, "");
...
export async function GET(
  ...
  const requestedPathRaw = req.nextUrl.searchParams.get("path");
  let requestedPath: string;
  try {
    requestedPath = normalizeRequestedPath(requestedPathRaw);
  } catch {
  ...
  try {
    ...
    try {
      ...
      if (readErr && readErr.code === "ENOENT") {
        // zip 直下に index.html が無い場合は ZIP 内を探索して index.html / index.htm を探す
    ...
    // HTML の場合もそのまま返却する（JS 実行は iframe sandbox="allow-scripts" で制御）
    ...
    console.error("trial-file error", err);
    return NextResponse.json(
